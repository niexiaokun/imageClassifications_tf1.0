...
from tf_cnn_net import nasnet
from tf_cnn_net import nasnet_utils
...
if __name__ == "__main__":
    ...
    with tf.device('/cpu:0'):
        with slim.arg_scope(pnasnet_mobile_arg_scope()):
            logits, end_points = build_pnasnet_mobile(inputs, 1001, is_training=False, final_endpoint='Cell_5')
    pprint(end_points)
    pprint(tf.contrib.graph_editor.get_tensors(tf.get_default_graph()))
    init_fn = slim.assign_from_checkpoint_fn(
        "/media/kun/4DDAE1651159A0A8/tf_object_models/pnasnet-5_mobile_2017_12_13/model.ckpt",
